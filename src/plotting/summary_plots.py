...
def plot_with_stats(data, metric, plot_type="violin"):
    ...
    if plot_type == "violin":
        ax = sns.violinplot(data=filtered_data, x="Group", y=metric, inner="quartile", palette="muted")
    elif plot_type == "box":
        ax = sns.boxplot(data=filtered_data, x="Group", y=metric, palette="muted")
    elif plot_type == "swarm":
        ax = sns.swarmplot(data=filtered_data, x="Group", y=metric, palette="muted", dodge=True)
    elif plot_type == "bar":
        ax = sns.barplot(data=filtered_data, x="Group", y=metric, ci="sd", palette="muted")
    elif plot_type == "point":
        ax = sns.pointplot(data=filtered_data, x="Group", y=metric, ci="sd", palette="muted", dodge=True)
...
def load_experiment_csv(file_path):
    ...
    metrics = [
        "Active_Neuron_Count",
        "Active_Neuron_Proportion",
        "Total_Estimated_Spikes",
        "SC_Avg_Instantaneous_Firing_Rate(Hz)"
    ]
    groups = data["Group"].unique().tolist()
