...
def return_baseline_F(F_file, Fneu):
    ...
    Args:
    -----------
        F_file : str, Path-like Object
            File path pointing to F.npy file generated by Suite2p
...
def filter_cascade_predictions(prediction_deltaF_file, config):
    ...
    Returns:
    -----------
        cascade_prediction : 2D NumPy Array
...
def basic_stats_per_cell(prediction_deltaF_file):
    """
    Calculates mean, standard deviation, and coefficient of variance for cells (rows) and time_points (columns) from
    Cascade-predicted activity (predictions_deltaF.npy) files. The first and last 32 frames cannot be processed by Cascade
    due to the nature of the deep-learning algorithm used for deconvolution
    ...
    Returns:
    -----------
        avg_instantaneous_spike_rate : float
            average activity per ROI
        avg_cell_sds : float
            standard deviation of activity per ROI
        avg_cell_cvs : float
            coefficient of variance of activity per ROI
        avg_time_stamp_mean : float
            average activity per imaged frame
        avg_time_stamp_sds : float
            standard deviation of activity per imaged frame
        avg_time_stamp_cvs : float
            coefficient of variance of activity per imaged frame    
    """
    # cell_means = []
    cell_sds = []
    cell_cvs = []
    ...
    for col_idx in range(frames):
        col_data = prediction_deltaF_file[:, col_idx]
...
def basic_estimated_stats_per_cell(prediction_deltaF_file):
    """
    Calculate the mean, standard deviation and coefficient of variance by ROI (cell).

    Args:
    -----------
        prediction_deltaF_file : 2D NumPy array
            2D NumPy cascade predictions
    
    Returns:
    -----------
        means : list
            list of each ROI's predicted activity mean
        sds : list
            list of each ROI's predicted activity standard deviation
        cvs : list
            list of each ROI's predicted activity coefficient of variance
    """
    means = []
    sds = []
    cvs = []
...
def summed_spike_probs_per_cell(prediction_deltaF_file):
    """
    Calculate the total number of Cascade-predicted action potentials / spikes that occur for each ROI
    ...
    Returns:
    -----------
        summed_spike_probs_cell : 1D numpy array
            np.nansum() of the total acitvity of a trace; cumulative spike count per ROI    
    """
...
def calculate_normalized_deltaF(F_file, config, baseline_correction = None, lambda_window = None, event_threshold = None, by_type = "Cell"):
    """
